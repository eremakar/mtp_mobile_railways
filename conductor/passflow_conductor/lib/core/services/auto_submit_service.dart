...
class AutoSubmitService {
  ...
  static Future<void> trySubmitCachedAnswers() async {
    ...
    if (taskListTypeModel.isEmpty) return;
  ...
  static Future _updateTaskStatus(int status, int typeId) async {
    ...
    logger.i('📦 Статус задачи обновлён до currentStatus = 3');
  }
  ...
  static FutureOr<int> _fillAnswers(int? typeId) async {
    ...
    if (typeId == null) {
      logger.i("❌ typeId для typeId=$typeId не найден в Hive");
    ...
    if (form == null) {
      logger.i("❌ form для typeId=$typeId не найден в Hive");
      return Future.value(status);
    }

    final answers = answerBox.get(form.id);
    if (answers == null) {
      logger.i("❌ answerBox для formId=${form.id} не найден в Hive");
      return Future.value(status);
    }

    final formConfig = formTypeBox.get(typeId)?.configuration;
    if (formConfig == null) {
      logger.i("❌ Конфигурация формы не найдена");
      return Future.value(status);
    }
    ...
    try {
      ...
      if (online) {
        ...
        if (result != null) {
          ...
          form.save();
          if (kDebugMode) {
            logger.i('✅ Все ответы успешно отправлены');
          }
        }
      }
    } catch (e) {
      logger.i('❌ Ошибка при отправке: $e');
    }
