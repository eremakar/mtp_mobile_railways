      ...
      if (loaded != null) {
        ...
        for (final block in loaded.configuration.data) {
          for (final task in block.tasks) {
            final typeId = task.typeId;
            if (typeId.isNotEmpty && !formTypeBox.containsKey(typeId)) {
              final parsedTypeId = int.tryParse(typeId);
              if (parsedTypeId == null) continue;

              final taskForm =
                  await formRepository.getFromTaskByTypeId(parsedTypeId);
              if (taskForm != null) {
                await formTypeBox.put(typeId, taskForm);
              }
            }
          }
        }
