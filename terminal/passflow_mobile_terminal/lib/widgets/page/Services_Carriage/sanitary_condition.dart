...
class SanitaryStatePage extends StatefulWidget {
  const SanitaryStatePage({Key? key}) : super(key: key);

  @override
  State<SanitaryStatePage> createState() => _SanitaryStatePageState();
}

class _SanitaryStatePageState extends State<SanitaryStatePage> {
  final Color blue = const Color(0xFF0B66FF);
  final Color pillBg = const Color(0xFFE6F0FF); 

  final List<_SanitaryItem> items = [
    _SanitaryItem(title: 'Мойка вагона - наружная'),
    _SanitaryItem(title: 'Мойка вагона - внутренняя'),
    _SanitaryItem(title: 'Дезинфекция вагона'),
  ];
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, i) {
                  ...
                  return Row(
                    ...
                    children: [
                      // Текст и кнопка “Оценить качество”
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            ...
                            _QualityPill(
                              ...
                              fg: blue,
                              onTap: () async {
                                // TODO: открыть страницу оценки качества
                                // демо: переведём чекбокс в true
                                setState(() =>
                                    items[i] = item.copyWith(checked: true));
                              },
                            ),
                          ],
                        ),
...
class _SanitaryItem {
  ...
  _SanitaryItem copyWith({String? title, bool? checked}) => _SanitaryItem(
